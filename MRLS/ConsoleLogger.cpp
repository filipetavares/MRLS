	...
	const char	*helper_executable/*=NULL*/)
{
	...
	{	// no name was give , create name based on the current address+time
		...
		MultiByteToWideChar(CP_ACP, 0, m_name, -1, (LPWSTR)wc_m_name, 64);
	}
	else
	...
	{	// failure
		PIPE_ACCESS_OUTBOUND,		// read/write access, we're only writing...
			NULL;// MessageBox(NULL, TEXT("CreateNamedPipe failed"), TEXT("ConsoleLogger failed"), MB_OK);
		return -1;
	}
	...
	MultiByteToWideChar(CP_ACP, 0, cmdline, -1, (LPWSTR)wc_cmdline, MAX_PATH);
	BOOL bRet = CreateProcess(NULL, wc_cmdline, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi);
	if (!bRet)
	{	// on failure - try to get the path from the environment
		...
		if (path)
		{
			...
			bRet = CreateProcess(NULL, wc_cmdline, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi);
		}
		if (!bRet)
		{
			//MessageBox(NULL, TEXT("Helper executable not found"), TEXT("ConsoleLogger failed"), MB_OK);
			CloseHandle(m_hPipe);
			m_hPipe = INVALID_HANDLE_VALUE;
			return -1;
		}
	...
	{
		//MessageBox(NULL, TEXT("WriteFile failed(1)"), TEXT("ConsoleLogger failed"), MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe = INVALID_HANDLE_VALUE;
		return -1;
	...
	{	// Send buffer-size
		...
		if (cbWritten != strlen(buffer))
		{
			//MessageBox(NULL, TEXT("WriteFile failed(2)"), TEXT("ConsoleLogger failed"), MB_OK);
			DisconnectNamedPipe(m_hPipe);
			CloseHandle(m_hPipe);
			m_hPipe = INVALID_HANDLE_VALUE;
			return -1;
	...
	if (cbWritten != 1)
	{
		//MessageBox(NULL, TEXT("WriteFile failed(3)"), TEXT("ConsoleLogger failed"), MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe = INVALID_HANDLE_VALUE;
		return -1;
